...
4. If one envelope causes an error (e.g. a bad phone number), the sorter logs the
   problem and moves on to the next one — it doesn't stop and refuse to deliver
   everything else.
5. Each check of the inbox opens a fresh database connection and closes it when done.
   This keeps the connection clean and avoids any leftover state from the previous run.
6. Tests can hand in a fake SMS sender directly, so we never accidentally call Twilio
